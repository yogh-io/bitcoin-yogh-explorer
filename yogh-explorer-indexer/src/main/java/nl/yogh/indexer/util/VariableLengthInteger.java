...
public final class VariableLengthInteger implements Serializable {
  ...
  public VariableLengthInteger(final InputStream stream) throws IOException {
    final byte lengthByte = stream.readNBytes(1)[0];

    if (lengthByte <= 252) {
      value = lengthByte;
      byteSize = 1;
    } else if (lengthByte == 253) {
      value = NumberParseUtil.parseLong(stream.readNBytes(2));
      byteSize = 3;
    } else if (lengthByte == 254) {
      value = NumberParseUtil.parseLong(stream.readNBytes(4));
      byteSize = 5;
    } else { // Must be 255
      value = NumberParseUtil.parseLong(stream.readNBytes(8));
      byteSize = 9;
    }
  }
  ...
  public VariableLengthInteger(final long value) {
    switch (sizeOf(value)) {
    case 1:
    ...
    case 5:
      bytes = new byte[5];
      bytes[0] = (byte) 254;
      final byte[] encodeUint32 = NumberEncodeUtil.encodeUint32(value);
      System.arraycopy(encodeUint32, 0, bytes, 1, 4);
      break;
    default:
      bytes = new byte[9];
      bytes[0] = (byte) 255;
      final byte[] encodeUint64 = NumberEncodeUtil.encodeUint64(value);
